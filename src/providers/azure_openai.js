 ...
 * }
 */
export default function initAzureOpenAI({
	llm,
	dokuUrl,
	apiKey,
	environment,
	...
	llm.chat.completions.create = async function (params) {
		...
		if (streaming) {
			...
			const originalResponseStream = await originalChatCreate.call(
				this,
				params
			);
	...
	llm.completions.create = async function (params) {
		...
		if (streaming) {
			// Call original method
			const originalResponseStream = await originalCompletionsCreate.call(
				this,
				params
			);
